...
function JobSeekerPubP(){
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        if (true) {
          axios.get(`https://nagaconnect-iitbilai.onrender.com/jobseeker/pub-profile/${userId}`)
          .then(response => {
      ...
      {profile ? (
        <div className="profile-card">
          ...
          <div className="profile-details">
            <h2>{profile.name}</h2>
            {profile.jobType && <p><strong>Job Type:</strong> {profile.jobType}</p>}
            {profile.age && <p><strong>Age:</strong> {profile.age}</p>}
            {profile.sex && <p><strong>Sex:</strong> {profile.sex}</p>}
